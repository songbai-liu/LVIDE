...
public class LSCCEA extends Algorithm{
	
	private int populationSize_;//population size
	private int subPopSize_;//population size
	...
	private SolutionSet[] subPopulations_;
	...
	private SolutionSet externalArchive_;
	double[][] improvement_;
	int[][] group_;
	int[][] broadenGroup_;
	int gSize_;
	...
	public void initialization() throws JMException, ClassNotFoundException{
		...
		externalArchive_ = new SolutionSet();
		improvement_ = new double[numVar_][numObj_];
		
		numOfGroup_ = 10;
		subPopulations_ = new SolutionSet[numOfGroup_];
		subPopSize_ = populationSize_/numOfGroup_;
		for(int i=0; i<numOfGroup_;i++){
			subPopulations_[i] = new SolutionSet(subPopSize_);
		}
		...
		for (int i = 0; i < numOfGroup_; i++) {
			for(int j = 0; j < subPopSize_; j++){
				newSolution = new Solution(problem_);
				problem_.evaluate(newSolution);
				problem_.evaluateConstraints(newSolution);
				subPopulations_[i].add(newSolution);
				externalArchive_.add(newSolution);
			}
		} // for
		estimateIdealPoint(externalArchive_);
		normalizedPopulation(externalArchive_);
		if(externalArchive_.size() > populationSize_){
			externalArchive_ = new PopulationClassification(externalArchive_,numObj_,lamada1_).classification()[0];
		}
		
		for (int i = 0; i < numOfGroup_; i++) {
			normalizedPopulation(subPopulations_[i]);
		}
	}
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(generations_ < maxGenerations_){
			if(generations_ < 0.35*maxGenerations_){
				for(int g=0;g<numOfGroup_;g++){
					Solution child;
					int size = 3;
					Solution[] parents = new Solution[size];
					Vector<Integer> list = new Vector<Integer>();
					for(int p = 0; p < subPopSize_; p++){
						int r;
						int ss;
						int t;
						while(list.size() < size){
							t = PseudoRandom.randInt(0, subPopSize_-1);
				  			boolean flag = true;
				  			for (int i=0; i<list.size(); i++) {
				  				if(list.get(i)==t || t==p){// t is in the list or t is equal to p
				  					flag = false;
				  					break;
				  				}
				  			}
				  			if(flag){// if (flag) list.push_back(t);
				  				list.addElement(t);
				  			}
				  		}
						parents[0] = subPopulations_[g].get(list.get(0));
						parents[1] = subPopulations_[g].get(list.get(1));
						parents[2] = subPopulations_[g].get(list.get(2));
						child = (Solution) crossoverOperator1_.execute(new Object[]{subPopulations_[g].get(p), parents, group_[g]});
						mutationOperator_.execute(new Object[]{child,group_});
						//STEP3. Function Evaluation
						problem_.evaluate(child);
						problem_.evaluateConstraints(child);
						boolean flag = updateIdealPoint(child);
						updateSubPopulation(subPopulations_[g], child);
						offspringPopulation_.add(child);
						//updateSubPopulation(externalArchive_, child, flag);
					}
				}
				...
				for(int g=0;g<numOfGroup_;g++){
					...
					Solution parent;
					for(int p = 0; p < subPopSize_; p++){
						child = new Solution(subPopulations_[g].get(p));
						XReal Xchild = new XReal(child);
						for(int gg=0;gg<numOfGroup_;gg++){
							if(gg != g){
								int rnd = PseudoRandom.randInt(0, subPopSize_-1);
								parent = subPopulations_[gg].get(rnd);
								XReal Xparent = new XReal(parent);
								int dim = group_[gg].length;
								for(int d=0;d<dim;d++){
									int var = group_[gg][d];
									double value = Xchild.getValue(var) + 0.5*(Xparent.getValue(var)-Xchild.getValue(var));
									if (value < lower_[var])
										value = lower_[var];
									if (value > upper_[var])
										value = upper_[var];
									Xchild.setValue(var, value);
								}
							}
						}
				...
				for(int g=0;g<numOfGroup_;g++){
					...
					for(int p = 0; p < subPopSize_; p++){
						...
						parents[1] = subPopulations_[g].get(list.get(1));
						parents[2] = subPopulations_[g].get(list.get(2));
						child = (Solution) crossoverOperator1_.execute(new Object[]{subPopulations_[g].get(p), parents, broadenGroup_[numOfGroup_-1]});
						mutationOperator_.execute(new Object[]{child,group_});
						//STEP3. Function Evaluation
						problem_.evaluate(child);
						problem_.evaluateConstraints(child);
		...
		NondominatedRanking final_ranking = new NondominatedRanking(externalArchive_);
	...
	public void reproduction(int[] group) throws JMException{
		...
		for (int i = 0; i < populationSize_; i++) {
			...
			int rnd2 = PseudoRandom.randInt(0, winnerSize-1);
			int rnd3 = PseudoRandom.randInt(0, loserSize-1);
			while(rnd1 == rnd2){
			...
			parents[2] = matingSet[1].get(rnd3);
			//STEP2. Reproduction: Crossover + Mutation
			Solution offSpring = (Solution) crossoverOperator1_.execute(new Object[]{population_.get(i), parents,group});
		...
		for (int i = 0; i < winnerSize; i++) {
			...
			if(i < winnerSize/5){
				...
				//STEP2. Reproduction: Crossover + Mutation
				offSpring = (Solution) crossoverOperator1_.execute(new Object[]{matingSet[0].get(i), parents,group});
			}
			mutationOperator_.execute(new Object[]{offSpring,group_});
			problem_.evaluate(offSpring);
			problem_.evaluateConstraints(offSpring);
	...
	public void environmentalSelection(){
		...
		unionPopulation_ = ((SolutionSet) externalArchive_).union(offspringPopulation_);
		externalArchive_.clear();
		...
		externalArchive_ = new PopulationClassification(unionPopulation_,numObj_,lamada1_).classification()[0];
	}
	...
	public void variablesDetection() throws ClassNotFoundException, JMException{
		...
		*/
		for(int n=0;n<numObj_;n++){
			...
			//evaluations_++;
			externalArchive_.add(new Solution(sol));
     ...
     */
  	...
  	private void updateSubPopulation(SolutionSet subPopulations_, Solution child){
  		normalizedPopulation(subPopulations_);
  		normalizedSolution(child);
  		int nearestID = 0;
  		double minAngle = computeAngle(subPopulations_.get(0), child);
  		double angle;
  		for(int i=1;i<subPopulations_.size();i++){
  			angle = computeAngle(subPopulations_.get(i), child);
  			if(angle < minAngle){
  				minAngle = angle;
  				nearestID = i;
  			}
  		...
  		int flag = dominance_.compare(child, subPopulations_.get(nearestID));
  		double value1 = child.getSumValue();
  		double value2 = subPopulations_.get(nearestID).getSumValue();
  		if(value1 < value2){
  			subPopulations_.replace(nearestID, child);
  		}
  		
  		/*if(flag == 1){ // The solution to add is dominated
			return; // Discard the new solution
		}else if(flag == -1){  // the solution in the archive is dominated
			subPopulations_.replace(nearestID, child); // Remove it from the population
		}else {
			if (PseudoRandom.randDouble() < 0.5){  // There is an equal
				subPopulations_.replace(nearestID, child);                      // Discard the new solution
			} // if
		}*/
  	}
  	
  	private boolean updateIdealPoint(Solution sol){
  		boolean flag = false;
  		for(int i=0; i<numObj_;i++){
  			if(sol.getObjective(i) < zideal_[i]){
				zideal_[i] = sol.getObjective(i);
				flag =  true;
			}//if
		}//for
  		return flag;
  	}
  	private void normalizedSolution(Solution sol){
		int minIndex = 0;
		double sum = 0.0;
		double normal = 0.0;
		for(int j=0; j<problem_.getNumberOfObjectives();j++){
