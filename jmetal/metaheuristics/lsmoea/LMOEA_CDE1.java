...
public class LMOEA_CDE1 extends Algorithm{
	...
	public LMOEA_CDE1(Problem problem) {
		super(problem);
	...
	public void initialization() throws JMException, ClassNotFoundException{
		...
		if(numVar_ == 100){
			...
			subGenerations_ = 40;
		}else if(numVar_ == 5000){
			numOfGroup_ = 100;
			subGenerations_ = 8000;
		}else{
			numOfGroup_ = 20;
			subGenerations_ = 10000;
		}
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		initialization();
		int initEvaluations = generations_+1;
		int T_ = 0;
		int[] inGroup = group_[0];
		while(generations_ < maxGenerations_){
			if(T_ < numOfGroup_-1){
				reproduction1(inGroup);
				if((generations_+1)%subGenerations_ == 0){
					T_++;
					/*int[] groupA = inGroup;
