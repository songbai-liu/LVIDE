...
public class LVIDE_Analysis_Importance extends Algorithm{
	...
	private Operator mutationOperator1_;
	private Operator mutationOperator2_;
	...
	public LVIDE_Analysis_Importance(Problem problem) {
		super(problem);
	...
	public void initialization() throws JMException, ClassNotFoundException{
		...
		//Read the operators
		mutationOperator1_ = operators_.get("mutation1");
		mutationOperator2_ = operators_.get("mutation2");
		...
		if(numVar_ <= 50){
			...
			numOfGroup_ = numVar_/20;
		}else if(numVar_ <= 1000){
			numOfGroup_ = numVar_/25;
		}else{
			numOfGroup_ = numVar_/50;
		}
		...
		printGroup(problem_.getName() + "_M5_100D_NatureOrder1.txt", group_);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(generations_ <= maxGenerations_){
			if(generations_ <= p*maxGenerations_){
				...
				reproduction_GDE(broadenGroup_[index]);
			}
	...
	public void reproduction_GDE(int[] group) throws JMException{
		...
		for (int i = 0; i < populationSize_; i++) {
			...
			if(numVar_ <= 500){
				mutationOperator2_.execute(offSpring);
			}else{
				mutationOperator1_.execute(new Object[]{offSpring,group_});
			}
		...
		*/
		for(int n=0;n<numObj_;n++){
			...
			for(int var=0;var<numVar_;var++){
				//int d = permutation[var];
				int d = var;
			//for(int d=numVar_-1;d>=0;d--){
	...
	//printGroup(problem.getName() + "_M5_100D.txt", group);
	public void printGroup(String path,int[][] group){
	    try {
	      /* Open the file */
