...
public class LMOEA_DE4 extends Algorithm{
	...
	public LMOEA_DE4(Problem problem) {
		super(problem);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(evaluations_ < maxEvaluations_){
			...
			if(T_ < numOfGroup_-1){
				if((evaluations_ - initEvaluations)%10000 == 0){
					T_++;
					int[] groupA = inGroup;
	...
	public void reproduction(int[] group) throws JMException{
		...
		for (int i = 0; i < populationSize_; i++) {
			...
			if(population_.get(n).getRank() == 0){
				double value = population_.get(n).getCrowdingDistance_();
				SolutionSet winnerSet = new SolutionSet();
				for(int j=0;j<populationSize_;j++){
					if(j != n){
						if(population_.get(j).getCrowdingDistance_() > value){
							winnerSet.add(population_.get(j));
						}
					}
				}
				
				if(winnerSet.size()>0){
					int rd = PseudoRandom.randInt(0, winnerSet.size()-1);
					parents[0] = winnerSet.get(rd);
				...
				int rd1 = PseudoRandom.randInt(0, population_.get(n).getRank()-1);
			    int rd2 = PseudoRandom.randInt(0, ranking.getSubfront(rd1).size()-1);
				parents[0] = ranking.getSubfront(rd1).get(rd2);
				/*int rd3 = PseudoRandom.randInt(0, population_.get(n).getRank()-1);
			    int rd4 = PseudoRandom.randInt(0, ranking.getSubfront(rd3).size()-1);*/
				parents[1] = population_.get(n);
			}
