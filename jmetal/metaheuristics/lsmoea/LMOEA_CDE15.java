...
public class LMOEA_CDE15 extends Algorithm{
	...
	public LMOEA_CDE15(Problem problem) {
		super(problem);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(generations_ <= maxGenerations_){
			if(generations_ <= p*maxGenerations_){
				...
				reproduction_GSBX(group_[index]);
			}else{
				int r1 = PseudoRandom.randInt(0, numOfGroup_-1);
				int r2 = PseudoRandom.randInt(0, numOfGroup_-1);
				while(r1 == r2){
				...
				reproduction_GSBX(broadenGroup_[index]);
			}
	...
	public void reproduction_GSBX(int[] group) throws JMException{
		...
		for (int i = 0; i < populationSize_; i++) {
			...
			if(numVar_ < 500){
				mutationOperator2_.execute(offSpring[0]);
			}else{
				mutationOperator1_.execute(new Object[]{offSpring[0],group_});
			}
