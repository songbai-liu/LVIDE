...
public class PopulationClassification {
	...
	public SolutionSet[] classification(){
		...
		SolutionSet[] st = getStSolutionSet(population_,size_);
		
		associating(st[0], clusters);
		int remain = size_;
		...
		while(remain > 0){
			...
			if(remain >= candidateSet.size()){
				...
				candidateSet.sort(new CrowdingDistanceComparator());
				for (int k = 0; k < remain; k++) {
					winner_.add(candidateSet.get(k));
				}
				for (int k = remain; k < candidateSet.size(); k++) {
					loser_.add(candidateSet.get(k));
				}
				break;
			}	
		}//while	
	...
	public SolutionSet[] getStSolutionSet(SolutionSet ss,int size) {
		...
		SolutionSet[] mgPopulation = new SolutionSet[2];
		mgPopulation[0] = new SolutionSet();
		mgPopulation[1] = new SolutionSet();
		front = ranking.getSubfront(index);
		while ((remain > 0) && (remain >= front.size())) {
			for (int k = 0; k < front.size(); k++) {
				mgPopulation[0].add(front.get(k));
			} // for
			// Decrement remain
		...
		while(index < ranking.getNumberOfSubfronts()-1){
			index++;
			front = ranking.getSubfront(index);
			for (int k = 0; k < front.size(); k++) {
				mgPopulation[1].add(front.get(k));
			}
		}
		return mgPopulation;
	}
