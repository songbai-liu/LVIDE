...
public class LMOEA_CDE4 extends Algorithm{
	...
	public LMOEA_CDE4(Problem problem) {
		super(problem);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		initialization();
		int inter = 5*maxGenerations_/(10*numOfGroup_);
		int T = 0;
		while(generations_ < maxGenerations_){
			if(T < numOfGroup_-1){
				if((generations_+1)%inter == 0){
					T++;
				}
				reproduction2(group_[T]);
			}else{
				reproduction(broadenGroup_[T]);
			}
