...
public class LMOEA_CDE5 extends Algorithm{
	...
	public LMOEA_CDE5(Problem problem) {
		super(problem);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		int inter1 = sb1*maxGenerations_/(10*numOfGroup_);
		int inter2 = sb2*maxGenerations_/(10*numOfGroup_);
		for(int i=0; i<numOfGroup_; i++){
			for(int G=0;G<inter1;G++){
				reproduction(group_[i]);
		...
		for(int G=0;G<500;G++){
			reproduction(broadenGroup_[numOfGroup_/2-1]);
			environmentalSelection();
			generations_++;
		}*/
		
		/*for(int i = 1; i<numOfGroup_; i++){
			
		}*/
		/*while(generations_ < maxGenerations_){
			for(int i=0; i<numOfGroup_; i++){
				for(int G=0;G<inter;G++){
					reproduction(group_[i]);
					environmentalSelection();
					generations_++;
				}
		...
		double[] p = new double[numOfGroup_];
		
		while(generations_ < 0.4*maxGenerations_){
			/*for(int i=0; i<numOfGroup_; i++){
				p[i] = PseudoRandom.randDouble(0, 1);
			}
			
			int[] group = getCurrentGroup(p);*/
		...
		while(generations_ < maxGenerations_){
			int rnd = PseudoRandom.randInt(0, numOfGroup_-1);
			reproduction(broadenGroup_[rnd]);
			environmentalSelection();
			generations_++;
		}
