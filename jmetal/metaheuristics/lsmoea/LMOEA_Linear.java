...
public class LMOEA_Linear extends Algorithm{
	...
	public LMOEA_Linear(Problem problem) {
		super(problem);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(evaluations_ < maxEvaluations_){
			/*if(T_ < numOfGroup_){
				reproduction(group_[T_]);
				if((evaluations_)%15000 == 0){
					T_++;
				}
			}else{
				reproduction(allVariables_);
	...
	public void variablesDetection() throws ClassNotFoundException, JMException{
		...
		Solution sol;//solutions for detection
		XReal individual;
		...
		*/
		...
		/*for(int d=0;d<numVar_;d++){
			for(int m=0;m<numObj_;m++){
				improvement_[d][m] = improvement[d][m];
			}
		}	*/
	}//variable detection
