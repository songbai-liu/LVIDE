...
public class OKA1 extends Problem {
	...
	public OKA1(String solutionType) {
		...
		upperLimit_[0] = 6 * Math.sin(Math.PI / 12.0) + 2 * Math.PI
	...
	public void evaluate(Solution solution) throws JMException {
		...
		double x0 = Math.cos(Math.PI / 12.0) * x[0] - Math.sin(Math.PI / 12.0)
				* x[1];
		double x1 = Math.sin(Math.PI / 12.0) * x[0] + Math.cos(Math.PI / 12.0)
				* x[1];

		fx[0] = x0;
		fx[1] = Math.sqrt(2 * Math.PI) - Math.sqrt(Math.abs(x0)) + 2
				* Math.pow(Math.abs(x1 - 3 * Math.cos(x0) - 3), 1.0 / 3.0);
