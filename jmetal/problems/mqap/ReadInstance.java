...
public class ReadInstance {
	...
	public void loadInstance() {
		...
		try {
			...
			while (st.hasMoreTokens()) {
				...
				if (newToken.toUpperCase().contains("FACILITIES")) {// facilities
																	// found
					newLine += newToken + " ";
					String aux = "";
					do {
			...
			Logger.getLogger(ReadInstance.class.getName()).log(Level.SEVERE,
					"The file doesn't exist", ex);
		} catch (IOException ex2) {
			Logger.getLogger(ReadInstance.class.getName()).log(Level.SEVERE,
					"Error reading from file", ex2);
		}
	} // loadInstance

	/**
	 * @author Juan J. Durillo This methods creates a single-objective instance
	 *         using a weighted aggregative approach and write all the
	 *         information to file
	 * @param vector
	...
	public void createSingleObjectiveInstance(int[] weights) {
		...
		try {
			 ...
			 * String newFirstLine = ""; for (int k = 0; k < objectives_; k++) {
			 * newFirstLine += " weight " + k + " " + weights[k]; }
			 * bw.write(singleObjectiveFirstLine_+ newFirstLine); bw.newLine();
			 */
			...
			Logger.getLogger(ReadInstance.class.getName()).log(Level.SEVERE,
					"The file cannot be created", ex);
		} catch (IOException ex1) {
			Logger.getLogger(ReadInstance.class.getName()).log(Level.SEVERE,
					"Error writting in the file", ex1);
		}
	}
	...
	public static void main(String[] args) {
		...
		if (args.length > 1) {
			// second argument should indicate if we do want to generte the
			// aggregatives
			if (args[1].toUpperCase().contains("Y")) {
				if (ri.getNumberOfObjectives() > 3) {
					...
					try {
						...
						System.out
								.println("initUniformWeight: failed when reading for file containing the weight");
						e.printStackTrace();
					}
				} else {
					int a = 100, b = 0;
